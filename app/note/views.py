...
from flask import request, render_template, url_for, flash, redirect, current_app, jsonify
...
def upload_image():
    ...
    msg = u'上传出错'
    file_path = ''
    filename_prefix = 'note__{0}__{1}'.format(current_user.id, datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            realname = filename_prefix + str('__') + filename
            ...
            if size > max_size:
                ...
                if upload_result:
                    ...
                    success = False
                    msg = u'文件上传失败，请检查access or secret key 是否配置正确'
...
def show_user_notes(user, page):
    ...
    return render_template('index.html', title='home', isHome=True, isUserNote=True,
                           notes=pagination.items, pagination=pagination, page=page)
...
def show_search_notes():
    ...
    return render_template('index.html', title='search', isHome=True, isSearch=True, keyword=keyword,
                           notes=pagination.items, pagination=pagination, page=page)
...
def show_note(id):
    ...
    return render_template('detail.html', title=note.title, user=user, note=note, isDetail=True)
...
def show_user_note(user, id):
    ...
    # 确定当前用户为文章的作者
    if user != current_user.nick_name or note is None:
        return render_template('404.html', title='page not found')
...
def write():
    ...
    if request.method == 'POST' and form.validate():
        ...
        result = note.create()
        if result == 'success':
            flash(u'你已经完成了一个笔记. 「<a class="black-text" href="{0}">{1}</a>」'.format(getNoteUrl(note),
                                                                                   form.title.data), result)
        else:
            flash('保存中出错.', result)
...
def edit(id):
    ...
    # 判断当前用户是否为文章作者
    if note.user_id != current_user.id or note is None:
        return render_template('404.html', title='page not found')
    ...
    if request.method == 'POST' and form.validate():
        ...
        result = note.update()
        if result == 'success':
            flash(u'你已经修改了你的笔记. 「<a class="black-text" href="{0}">{1}</a>」'.format(getNoteUrl(note),
                                                                                             form.title.data), result)
        else:
            flash('保存中出错.', result)
